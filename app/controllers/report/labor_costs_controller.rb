...
class Report::LaborCostsController < Report::BaseController
  ...
  def show
    ...
    cspms = if @user_cost_type_id.present?
      ...
      positions.name position_name, positions.functional_category, users.locked_at, user_cost_types.name user_cost_type_name,
      sum(amount) total')
      .joins(:user, :position, :user_cost_type)
      .joins("LEFT JOIN `departments` ON `departments`.`id` = `positions`.`department_id` ")
      .group(:"users.clerk_code", :"users.chinese_name", :"departments.name", :"departments.company_name", :"positions.name", :"positions.functional_category", :"users.locked_at", :"user_cost_types.name")
      .order(:"users.clerk_code", :"users.chinese_name", :"departments.name", :"departments.company_name", :"positions.name", :"positions.functional_category", :"users.locked_at", :"user_cost_types.name")
    ...
    respond_to do |format|
      ...
      format.csv do
        ...
        csv_res = CSV.generate do |csv|
          csv << %w(工号 姓名 所属公司 所属部门 岗位 岗位类别 是否在职 成本性质 查询年月 金额)
          cspms.each do |d|
            values = []
            values << d.clerk_code
