  ...
  def show
    ...
    if @short_company_name.present?
      ...
      data = policy_scope(Bi::WorkHoursCountDetailDept).where(date: beginning_of_month..end_of_month).where(orgname: @company_name)
        ...
        .joins("LEFT JOIN ORG_REPORT_DEPT_ORDER on ORG_REPORT_DEPT_ORDER.组织编号 = WORK_HOURS_COUNT_DETAIL_DEPT.deptcode")
        .group("WORK_HOURS_COUNT_DETAIL_DEPT.deptcode, ORG_REPORT_DEPT_ORDER.显示顺序, deptname")
        .order("ORG_REPORT_DEPT_ORDER.显示顺序, WORK_HOURS_COUNT_DETAIL_DEPT.deptcode")
