    ...
    class Scope < Scope
      def resolve
        ...
        if user.roles.pluck(:report_view_all).any? || user.admin? || can_access_org_codes == User::ALL_OF_ALL
          ...
          scope.where(orgcode: can_access_org_codes, deptcode: user.can_access_dept_codes).or(scope.where(orgcode_sum: can_access_org_codes, deptcode_sum: user.can_access_dept_codes))
        else
          scope.none
        end
      end
